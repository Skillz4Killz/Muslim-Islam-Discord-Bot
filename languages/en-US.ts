import { KlasaClient, KlasaUtil, Language, LanguageStore } from '../imports';
    ...
    this.language = {
      ...
      COMMAND_BLACKLIST_SUCCESS: (
        ...
        [
          ...
          .filter((val) => val !== '')
          .join('\n'),
      COMMAND_CONF_GET: (key, value) =>
        `The value for the key **${key}** is: \`${value}\``,
      COMMAND_CONF_GET_NOEXT: (key) =>
        `The key **${key}** does not seem to exist.`,
      COMMAND_CONF_GUARDED: (name) =>
        `${KlasaUtil.toTitleCase(name)} may not be disabled.`,
      COMMAND_CONF_KEY_NOT_ARRAY:
        "This key is not array type. Use the action 'reset' instead.",
      COMMAND_CONF_NOCHANGE: (key) =>
        `The value for **${key}** was already that value.`,
      ...
      COMMAND_DISABLE: (type, name) =>
        `+ Successfully disabled ${type}: ${name}`,
      ...
      COMMAND_EVAL_DESCRIPTION:
        'Evaluates arbitrary Javascript. Reserved for bot owner.',
      ...
      COMMAND_EVAL_EXTENDEDHELP: [
        ...
        "The --depth flag accepts a number, for example, --depth=2, to customize KlasaUtil.inspect's depth.",
        'The --async flag will wrap the code into an async function where you can enjoy the use of await, however, if you want to return something, you will need the return keyword.',
        'The --showHidden flag will enable the showHidden option in KlasaUtil.inspect.',
      ...
      COMMAND_LOAD_ERROR: (type, name, error) =>
        `❌ Failed to load ${type}: ${name}. Reason:${KlasaUtil.codeBlock(
          'js',
          error
        )}`,
      COMMAND_LOAD_FAIL:
      ...
      COMMAND_RELOAD: (type, name, time) =>
        `✅ Reloaded ${type}: ${name}. (Took: ${time})`,
      COMMAND_RELOAD_ALL: (type, time) =>
        `✅ Reloaded all ${type}. (Took: ${time})`,
      COMMAND_RELOAD_DESCRIPTION:
        'Reloads a klasa piece, or all pieces of a klasa store.',
      COMMAND_RELOAD_EVERYTHING: (time) =>
        `✅ Reloaded everything. (Took: ${time})`,
      ...
      COMMAND_TRANSFER_DESCRIPTION:
        'Transfers a core piece to its respective folder.',
      COMMAND_TRANSFER_ERROR:
        '❌ That file has been transfered already or never existed.',
      ...
      PREFIX_REMINDER: (prefix = `@BOTMENTION`) =>
        `The prefix${
