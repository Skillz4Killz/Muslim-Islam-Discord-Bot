...
module.exports = class extends Task {
	...
	async run() {
		...
		const usersToRemind = this.client.users.filter(user => user.settings.reminders.finishMonthly.enabled).array();
		...
		for (const user of usersToRemind) {
			...
			const embed = new MessageEmbed()
				.setColor('RANDOM')
				.setAuthor(`Surah ${surah.name} Ayah #${ayah}`, user.displayAvatarURL())
				.setDescription(ayahToSend.text)
				.setImage(ayahToSend.image)
				.setTimestamp()
				.setFooter('Credits To Quran.com');

			const sentReminder = await this.client.channels.get('517175884432801795').send(user, { embed });
			if (sentReminder) await sentReminder.react('✅');

			const { errors } = await user.settings.update('reminders.finishMonthly.verse', verse === 6235 ? 1 : verse + 1);
